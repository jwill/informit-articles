...
@app.route('/login')
def login():
  flow = OAuth2WebServerFlow(client_id=myconfig.CLIENT_ID,
...
def index():
  ...
  return "<pre>"+pprint.pformat(timeline_item)
...
@app.route('/card/<int:cardid>')
def getCard(cardid):

def createCard(front, back):

def createCardForUser(userid, sourceCardId):
  # retrieve card from database
  # add card to database indexed by user
  return

# cardId - id of the generated card
# cardType - show FRONT, BACK, or ALL
def updateCard(cardId, cardType):
